...
export default function AssistantPanel({ data }) {
  ...
  const ask = (q) => {
    ...
    let a = `There are ${data.stats.products} products.`;
    if (text.includes("order"))
      a = `There are ${data.stats.orders} orders in the workspace.`;
    if (text.includes("customer"))
      a = `There are ${data.stats.customers} customers.`;
    ...
    if (text.includes("review"))
      a = `Latest review: “${data.reviews[0]?.body || "No reviews yet."}”`;
    setAnswer(a);
  };
  return (
    <>
        ...
        {open && (
          ...
          >
            <motion.section
              ...
              className="w-full max-w-lg rounded-3xl border border-[var(--line)] bg-[var(--surface)] p-5 shadow-2xl sm:p-6"
            >
              ...
              <div className="my-5 rounded-2xl bg-[var(--bg)] p-4 text-sm leading-6">
                Ask a quick question about your live workspace.
              </div>
              <div className="flex flex-wrap gap-2">
                {suggestions.map((s) => (
                  <button
                    onClick={() => ask(s)}
                    key={s}
                    className="rounded-lg border border-[var(--line)] px-3 py-2 text-left text-xs transition hover:border-[var(--primary)]"
                  >
                    {s}
                  </button>
                ))}
              ...
              {answer && (
                <div className="mt-5 rounded-2xl bg-[var(--soft)] p-4 text-sm">
                  {answer}
                </div>
              )}
              <form
              ...
              >
                <input
                  name="question"
                  className="min-w-0 flex-1 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-3 text-sm outline-none focus:border-[var(--primary)]"
